...
수준 표기(정직): 자율 에이전트가 아니다. 경계가 명시된 L2 루프이며,
도구 선택(function calling)·상태기계·계획 수립은 없다.

세는 단위를 섞으면 숫자가 어긋나 보이므로 둘 다 적는다:
  · **모델에게 판정을 맡기는 호출은 2회** — 관련성(grade)과 근거점검(grounded).
    '재작성 발동'은 관련성 판정에서 파생되지 별도 호출이 아니다.
  · **판단 지점으로 세면 3개** — 관련성 / 재작성 여부 / 근거점검.
  · 어느 쪽으로 세든 **실제로 제어를 바꾸는 분기는 1개**(관련성), 재시도 상한은 1회다.
