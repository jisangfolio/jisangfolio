...
NODES = [
    ...
    {"id": "ghci", "group": "skill", "ko": "CI 회귀 게이트", "en": "CI regression gates",
     "desc_ko": "GitHub Actions + pytest 스위트를 Python 3.11·3.12 매트릭스로 실행. 코드뿐 아니라 **문서의 진실성**까지 게이트 — 코드그래프 스테일 검사, 골든셋 개수 대 문서 대조, MCP 사본 드리프트, README 배지 대 실제 요구 버전. CI는 키 없이 도는 LLM-free 레이어만 검증한다는 범위도 명시.",
