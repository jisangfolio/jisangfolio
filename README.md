...
> Module-import graph, auto-derived from the codebase (GitHub renders this natively). An interactive, function-level version (vis-network, 220 nodes) is embedded on the [live site](https://jisangfolio.streamlit.app) and in `assets/codegraph.html` (regenerated by `gen_codegraph.py`).
...
- **Regression eval harness** — `evals/` scores the chatbot, the router, and the Agentic RAG path with deterministic checks (fact keywords · banned terms · retrieval hits) + an LLM-as-judge (a *different* model, to avoid self-scoring bias). It once caught a stale résumé copy leaking into the bot, taking the chatbot golden set from **10/16 to 15/16**. Read that as a before/after on one fixed set, not a benchmark: n=16 is a wide interval (McNemar p≈0.06). The golden set is **48 cases** today — 19 chat · 20 router · 9 RAG — and the 16 above is the chat set as it stood at that measurement, not its current size.
...
> Re-measuring is rate-limited rather than hard. The deployed app and the harness share one Groq key and one 200k/day budget, and a full 48-case sweep costs ~169k of it — running the whole set hands the day's visitors a 429. So the harness now **defaults to a `core` subset** (~76k, 38%) and a pre-flight check refuses anything above 40% of the day's budget; `--full` exists but is gated. `core` keeps every case that only a live model can answer — injection, off-topic, persona guards — while the factual cases are covered at zero tokens by `tests/test_resume_facts.py`. The corrected figure will be filled in from measured runs, not estimated, and `evals/runs/` holds what has actually been scored so far.
