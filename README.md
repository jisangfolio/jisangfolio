...
> Module-import graph, auto-derived from the codebase (GitHub renders this natively). An interactive, function-level version (vis-network, 207 nodes) is embedded on the [live site](https://jisangfolio.streamlit.app) and in `assets/codegraph.html` (regenerated by `gen_codegraph.py`).
...
> Re-measuring is rate-limited rather than hard: a full 48-case run costs ~206k tokens against a 200k/day free-tier budget — one full sweep is more than a day's allowance, and the deployed app draws on the same key, so a full run would leave visitors seeing 429s. That is why 8 cases are tagged `core: true` and `--quick` runs only those (~42k). The corrected figure will be filled in from measured runs, not estimated, and `evals/runs/` holds what has actually been scored so far.
