...
- **프로필 지식그래프(SSOT)** — 학력·경력·프로젝트·스킬·학부 코스워크를 노드/엣지로 정의(`profile_graph.py`)해 홈에 인터랙티브 그래프로 임베드 + 같은 데이터를 챗봇 시스템 프롬프트에 구조 요약으로 주입 → 그래프·챗봇이 단일 소스 공유
...
├── profile_graph.py            # 프로필 지식그래프 SSOT (홈 그래프 + 챗봇 주입)
├── ui.py                       # 공유 스타일(Pretendard 폰트·카드/버튼 라운딩)
...
- **프로필 지식그래프**: 학력·경력·프로젝트·스킬·학부 코스워크를 노드/엣지로 정의(`profile_graph.py`)한 인터랙티브 그래프를 홈에 임베드. 동일 데이터를 챗봇 시스템 프롬프트에 `[프로필 관계도]` 구조 요약으로 함께 주입 — 그래프와 챗봇이 하나의 소스를 공유
