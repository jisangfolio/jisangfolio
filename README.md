...
- **Data Analysis** — upload a CSV/Excel file and an LLM router decides between generating & executing pandas code (for aggregates; `exec` restricted to a 21-name builtins allowlist — not a real sandbox: no timeout, no process isolation) and hybrid retrieval — FAISS (dense) + BM25 (sparse) fused with RRF — for search, with automatic RAG fallback on code failure.
...
- **Retrieval self-diagnosis (measured, not claimed)** — `retrieval_probe.py` measures the retrieval layer's own defects instead of asserting quality. Two it surfaced: the embedder (`all-MiniLM-L6-v2`, 256 word-piece limit) silently truncated long chunks — worst for Korean, which tokenizes into far more pieces — and one document dominates the corpus. Shrinking chunks 1200 → 500 chars lifted the share of each chunk that actually reaches the encoder from **53% → 81%** (English chunks 88% → 99%) and cross-lingual retrieval from 2/5 → 4/5 probe queries, with golden-set vendor hits unchanged (7/7). Residual, still true: **~82% of Korean chunks remain truncated** (a small-to-big retriever, or a multilingual encoder, is the real fix) and the on-prem KETI doc is **~89% of all chunks**, so corpus balance — not just ranking — bounds retrieval quality here.
