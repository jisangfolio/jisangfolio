...
> Module-import graph, auto-derived from the codebase (GitHub renders this natively). An interactive, function-level version (vis-network, 322 nodes) is embedded on the [live site](https://jisangfolio.streamlit.app) and in `assets/codegraph.html` (regenerated by `gen_codegraph.py`).
